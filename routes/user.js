...
router.post("/login",(req,res)=>{
    ...
    connection.query(sql, [username], function(err,results){
        if(err || results?.length == 0) return res.status(400).json({error: "Username or password incorrect!"})
        const user = results[0];
        bcrypt.compare(password, user.password, function(err, result) {
            if(!result) return res.status(400).json({error: "Username or password incorrect!"});
...
router.post("/signup",(req,res)=>{
    if(!req.body.username ||  !req.body.password || !req.body.email) return res.status(400).json({error: "Username, password or email missing!"})
...
router.get("/conversations/:user", (req, res)=>{
    ...
    connection.query(sql, [user,user], function(err,results){
        if(err || results.length === 0) return res.status(400).json({error: "No users found!"})
        let allUsers = [];
