...
router.get("/conversations/:user", (req, res)=>{
    ...
    const sql = "SELECT m.content, latest.username FROM messages m JOIN (SELECT MAX(`id`) AS `id`, CASE WHEN m.from = ? THEN m.to WHEN m.from = ? THEN m.to END AS `username` from messages m GROUP BY `username`) latest on m.id=latest.id GROUP BY `username` HAVING username IS NOT NULL";   
