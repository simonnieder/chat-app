const Input = ({value, onChange, placeholder, title, full, type="text", error, required, search}) => {
    return (
        <div className={`${full ? "w-full" : "max-w-sm w-full"} flex flex-col`}>
            ...
            <p className="font-inter font-normal text-red-500 text-xs">{error}</p>
        </div>
    ) 
}
