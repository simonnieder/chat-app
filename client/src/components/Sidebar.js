...
const Sidebar = ({ id, isBigScreen, refresh }) => {
  ...
  return (
    <div
      className={`${
        isBigScreen && "max-w-xs"
      } w-full bg-blue-gray-500 bg-opacity-10 space-y-2 flex flex-col`}
    >
      <div className="flex items-center justify-between h-20 px-5 border-b border-gray-300 flex-shrink-0">
        <div className="flex items-center">
          ...
          <span className=" text-gray-700 font-roboto font-medium text-3xl ml-3">
            {user.username}
          </span>
        </div>
      </div>
      <div className="space-y-3 flex flex-col min-h-0 p-5 h-full">
        ...
        <div className="">
          <Button outlined full onClick={logout}>
            logout
          </Button>
        </div>
      </div>
